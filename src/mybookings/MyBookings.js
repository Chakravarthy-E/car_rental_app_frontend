...
const MyBookings = () => {
  ...
  const { bookingDetails, setBookingDetails, verifieduser, setverifieduser } = useContext(CarContextDetails);
  ...
  useEffect(()=>{
    ...
    async function Mycart(){
      ...
      try{
        const data = await axios.get("http://localhost:5000/getallbookcar",{ headers })
  ...
  const handleEdit = (booking) => {
    ...
    const editcardetails=window.localStorage.setItem('usereditbooking', JSON.stringify(booking));
  ...
  const handleCancel = async (bookingid) => {
    ...
    try{
      const data = await axios.post("http://localhost:5000/deletecar",{ bookingid} ,{headers})
      console.log(data);
