...
const SplitForm = () => {
  ...
  useEffect(() => {
    ...
    console.log(cloudinary, public_id,filter)
  }, [cloudinary, public_id,filter])
  ...
  useEffect(() => {
    ...
    async function adminvar() {
      ...
      try {
        ...
        const adminvar = await axios.get("http://localhost:5000/adminAuth", { headers });
...
//   Final Publicid Part
  ...
  useEffect(() => {
    ...
    async function sendDataToServer() {
      ...
      try {
        ...
        const data = await axios.post("http://localhost:5000/editadmincar", filter
        ,{
          params: {
            images:public_id
          },
  ...
  const handleImageUpload = async (e) => {
    ...
    if (file) {
      ...
      try {
        const data = await axios.post('http://localhost:5000/api/deleteimage', object, { headers })
        console.log(data)
      }
      catch (error) {
...
// Final image upload to cloudinary
  ...
  const handleDelete = async (e) => {
    ...
    try {
      const data = await axios.post('http://localhost:5000/api/deleteimage', object, { withCredentials: true })
      console.log(data)
    }
    catch (error) {
    ...
    try {
      const data = await axios.post('http://localhost:5000/deleteadmincar', { _id }, { headers })
      console.log(data);
      ...
      for (let prop in carDetails) {
        ...
        if (carDetails[prop] && prop != '_id') {
          ...
          try {
                ...
                const updatedata = await axios.post("http://localhost:5000/editadmincar", update, { headers })
